...
input_file_path = "gs://etl-proj-ash/raw/users/*"  # Use a wildcard to handle multiple files
...
df = df.withColumn("churn", col("churn").cast("boolean"))
df = df.withColumn("creation_date", to_timestamp(col("creation_date"), "yyyy-MM-dd HH:mm:ss"))    
df = df.withColumn("age_group", col("age_group").cast("float"))
df = df.withColumn("gender", col("gender").cast("float"))
df = df.withColumn("last_activity_date", to_timestamp(col("last_activity_date"), "yyyy-MM-dd HH:mm:ss")) 
...
df.write \
    .format("bigquery") \
    .option("table", "etl-proj-433402.dataprocetl.users") \
